...
- `variants/<slug>/` — classic-keyboard color variants (IBM Model M,
  Macintosh Plus, Commodore 64, amber terminal): each is its own complete,
  self-contained theme package (own `theme.txt`/`id`/assets), generated by
  `scripts/generate_variants.py`. FUTO's theme format has no in-app
  palette switching within one theme (one `theme.txt` = one fixed
  `[colors]`/asset set) — these are separate installable packages, not a
  mode inside Groovy Code itself. See `docs/VARIANTS.md`.
